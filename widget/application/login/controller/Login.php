...
class Login extends Controller 
{
 ...
 public function get_Invitation_mode(){
	...
	$data2=	db('ikmovie')->where('id',1)->find();
	$so =db('shezi')->where('id',1)->find();
	$data2['sotuij']=$so['sotuij'];
   if($data2){
	return  json(['msg'=>$data2]);
   }
 } 
 //首页右上角nickname_Ryan
 public function nickname_Ryan(){ 
    ...
    $xinxi=db('ikmovie')->where('id',1)->find();
	$_var_55=$xinxi['set_nickname_integral'];	
	$_var_56=db('user')->where('id',$data['uid'])->find();
    if ($_var_55 == '') {
			} else {
				if ($_var_56['nick_name'] == '') {
					db('user')->where('id', $data['uid'])->setInc('sign', $_var_55);
				}
			}
	  $so=db('user')->where('id',$data['uid'])->update(['nick_name'=>$nickname]); 
	 if($so){
	return  json(['code'=>1]);
   }else{
	 return  json(['code'=>0]);  
 ...
 //代理中心
  public function agent_settings(){ 
    ...
    $soi=db('shezi')->where('id',1)->find();
    if($data['key']!==$soi['ydkey']){
          return json(['code'=>999,'msg'=>'请勿非法操作']);
        }
   $so=db('user')->where('id',$data['uid'])->find();
   $arr['agent_money']=$so['money'];
   $arr['agent_weichat']=$so['weichat'];
   $yqm=$so['share_ma'];
   $arr['user_count']=db('user')->where('parentid',$yqm)->count();
   $arr['agent_count']=db('user')->where('parentid', $yqm and 'power',1)->count();
  ...
  public function agent_settings_weichat(){ 
	...
	if($data['weichat']==""){
		return json(['code'=>0,'msg'=>'不能为空！']);	
	}else{
		db('user')->where('id',$data['uid'])->update(['weichat'=>$data['weichat']]); 
 	...
 	public function userheadimg_Ryan()
	{
		...
		if ($_var_54 == $soi['ydkey']) {
			...
			db('user')->where('id', $_var_53['id'])->update(['userimg' => $_var_53['headimgurl']]);
			if ($_var_53) {
    ...
    $soi=db('shezi')->where('id',1)->find();
    if($data['key']!==$soi['ydkey']){
          return json(['code'=>718,'msg'=>'请勿非法操作']);
        }
	$so=db('user')->where('id',$data['uid'])->find();
	$_var_52['userimg'] = $so['userimg'];
	$_var_52['Upload_Picture_Size'] = 300000;
    if ($so) {
 ...
 public function Index_right(){ 
	...
	$Index_right=db('banner')->where('cid',12)->select();
   if($Index_right){
	return  json(['msg'=>$Index_right]);
   }
 }
 //公告消息zfbcz_Ryan
  public function zfbcz_Ryan(){
	...
	$xinxi='账户:'.$data['Alipay'].'，姓名：'.$data['Alipay_name'];
	$bb=db('user')->where('id',$data['id'])->update(['zfb'=>$xinxi]); 
	if($bb){
		$xinx=db('ikmovie')->where('id',1)->find();
		$jine=$xinx['upload_alipay_code_integral'];
		db('user')->where('id', $data['id'])->setInc('sign', $jine);
		return  json(['code'=>1,'msg'=>'成功！获得'.$jine.'积分！']);
	}else{
		return  json(['code'=>0,'msg'=>'失败！']);
	}
	  }
 
  public function ikgg(){ 
	$data = input();
    $soi=db('shezi')->where('id',1)->find();
    if($data['key']!==$soi['ydkey']){
          return json(['code'=>0,'msg'=>'请勿非法操作']);
	...
	$datas=db('shop')->find();
	$articallist['picture']=$datas['picurl'];
	$articallist['title']=$datas['title'];
	$articallist['id']=$datas['id'];
	$articallist['content']=$datas['miaoshu'];
	$articallist['url']=$datas['money'];
	$articallist['time']=$datas['time'];
    $b=array();
    $b=array($articallist);
    $articallist1['articallist']= $b;
   if($datas){
	return  json(['msg'=>$articallist1]);
   }
 } 
 //获取用户数据 
  public function frame_two(){ 
	$data = input();
    $so2=db('shezi')->where('id',1)->find();
    if($data['key']!==$so2['ydkey']){
          return json(['code'=>0,'msg'=>'请勿非法操作']);
        }
	$so=db('user')->where('id',$data['uid'])->find();
	$xinxi=db('ikmovie')->where('id',1)->find();
    $data1=array_merge($so,$xinxi);
   if($so){
	return  json(['msg'=>$data1]);
   }
   
 } 
  public function jbdh() {
		$data = input();
		$where['id'] = $data['id'];
		$dhts = $data['age'];
		...
		$total=$dhts*$arr['daybili'];
		if($arr['jinbi']<$total){
			return json(['code' => '0','msg'=>"金币不足"]);
		}else{
		   $shengyu=$arr['jinbi']-$total;
		   $time2=$dhts*24*60*60;
		   if($arr['time']<time()){
			   $timeend=time() + $time2;
		   }else{
			   $timeend=$arr['time']+$time2;
		   }
		 
		db('user')->where($where)->update(['lasttime'=>$timeend,'sign'=>$shengyu]);   
		return json(['code' => '1','msg'=>$dhts]); 
	 }
	 }
  //获取信息upgrade_vip 
  ...
  public function sign(){ 
    ...
    $so=db('user')->where('id',$data['uid'])->find();
	$_var_7 = db('user')->where(['parentid' => $data['uid']])->count();
	$so['tudi']=$_var_7;
	if($so){
	return  json(['msg'=>$so]);
   }
	...
	//fenx
  public function ikshare(){
	$data = input();
    $so2=db('shezi')->where('id',1)->find();
    if($data['key']!==$so2['ydkey']){
          return json(['code'=>0,'msg'=>'请勿非法操作']);
        }
    $_var_7 = db('user')->where(['id' => $data['uid']])->find();		
    $gettime=db('advert')->where('id',5)->find();
	$arr['Registered_Gift_Time']=$gettime['content'];
    $host=$_SERVER['SERVER_NAME'];	
	$jb=db('shezi')->where('id',1)->find();	
    $arr['Invitation_points']	=$jb['sharefjb'];
    $arr['host']	=$host;
	$arr['dwzapi']='http://'.$host.'/app/index/qudao.html?uid='.$_var_7['share_ma'];
	$bb= db('ikmovie')->where('id',1)->find();
	$arr['r'][0]= $bb['share_Poster_template'];
	$arr['r'][1]= $bb['share_Poster_template2'];
	$arr['r'][2]= $bb['share_Poster_template3'];
	 return json($arr);
	}  
...
db('cxjl')->where('uid',$username)->where('title',$spname)->update(['time' => $time]);
  ...
  public function logo() 
	{
       ...
       $dat = db('ikmovie')->where('id',1)->find();
        $arr['yyms'] = $datas['yyms'];
		$arr['id'] = $data['id'];
      ...
      $arr['faka_vip2'] = $dai3['url1'];
      }else{
      $arr['faka_vip2'] =$chongzhi['url1'];
      }
      if($dai3['url2']<>"无数据"|| !$dai3['url2']){
      $arr['faka_vip3'] = $dai3['url2'];
      }else{
      $arr['faka_vip3'] =$chongzhi['url2'];
      }
      if($dai3['url3']<>"无数据"|| !$dai3['url3']){
      $arr['faka_vip5'] = $dai3['url3'];
      }else{
      $arr['faka_vip5'] =$chongzhi['url3'];
      }
      if($dai3['url4']<>"无数据"|| !$dai3['url4']){
      $arr['faka_vip6'] = $dai3['url4'];
      }else{
      $arr['faka_vip6'] =$chongzhi['url4'];
      }
      if($dai3['url5']<>"无数据"|| !$dai3['url5']){
      $arr['faka_vip1'] = $dai3['url5'];
      }else{
      $arr['faka_vip1'] =$chongzhi['url5'];
      }
      if($dai3['url6']<>"无数据"|| !$dai3['url6']){
      $arr['faka_vip4'] = $dai3['url6'];
      }else{
      $arr['faka_vip4'] =$chongzhi['url6'];
      }
    ...
    $arr['vip2'] = $dai['yueka'];  
    $arr['vip3'] = $dai['jika'];  
    $arr['vip4'] = $dai['bannian'];  
    $arr['vip5'] = $dai['nianka'];
    $arr['vip6'] = $dai['yongjiu']; 
	$arr['daili_price'] = $dai['daili']; 
	$advert = db('advert')->where('id','20')->find();
	$arr['app_name']=$advert[content];
		if ($data) 
		{
  ...
  public function repasss1() 
    ...
    // $re_xinpass = $data['re_xinpass'];
        ...
        // if ($data['xinpass'] != $data['re_xinpass']) {
        // 	return json(['code' => 0, 'msg' => '确认新密码和新密码不相同，请重新输入']);
        // }
  ...
  //余额
	...
	//体现
	public function qbtx() 
	{
		$data = input();
		$where['id'] = $data['uid'];
		$txje = $data['shuji'];
		$data = db('user')->where($where)->find();
		$yongjinsu1=$data['yongjinsu'];
		if ($data['money'] < $txje){
			
          return json(['code' => '0','msg'=>"余额不足！"]);			
		}else{
			$yue= $data['money']-$txje;
			$yongjinsu2=$yongjinsu1+$txje;
			db('user')->where($where)->update(['ctime'=>time(),'txje'=>$txje,'money'=>$yue,'tx'=>'1','yongjinsu'=>$yongjinsu2]);
			 return json(['code' => '1','msg'=>$txje]);		
		}
...
//体现记录
    ...
    public function create1() 
	{
      ...
      $a2=$data['deviceId'];
		$select = db('user')->where('imeilogo',$a2)->count();
	...
	//	if ($data['password'] != $data['re_password']) {
		//	return json(['code' => 0, 'msg' => '确认密码和密码不相同']);
		//}
		...
		$insert['userimg'] = "http://cs.xiao18.top/images/images/logo.png";
		$insert['imeilogo'] = $a2;
  ...
  public function vlist() 
	{
		...
		return json(['code'=>$code,'zt_tv'=>$num,'tv'=>$num1]);
...
//		}
      ...
      return jsonp(['code'=>0,'msg'=>'手机号码已领取，请下载APP查看']);
		...
		$coun = db('user')->where('share_ma',$data['uid'])->find(); 
		if($coun){$newid=$coun['id'];}else{$newid='';}
      	$insert['username'] = $data['phone'];
		$insert['Source'] = '分享注册';      
		$insert['password'] = md5(sha1($data['pass']));
		$insert['power'] = '2';
		$insert['status'] = '1';
		$insert['parentid'] = $newid;
		$insert['ctime'] = time();
		$insert['logintime'] = '0';
		$insert['lasttime'] = time()+advert('5')*60;
		...
		$insert['share_ma'] = substr(base_convert(md5(uniqid(md5(microtime(true)),true)), 16, 10), 0, 6);
          ...
          $jinb = db('user')->where('id',$newid)->find();                              
          db('user')->where('id',$newid)->update(['sign' => (int)$jinb['sign'] + (int)$zhuce['sharefjb']]);                    

...
//        print_r(json_encode($_POST));
  	...
  	public function yhtx()
    {
        ...
        // $arr    =   implode(',',array_filter(explode(',',$id)));
				 // 
	 $ikmovie=db('txlog')->insert(['uid'=>$id,'time'=>time(),'nickname'=>input('nickname'),'status'=>'2','jine'=>input('txje'),'zfb'=>input('zfb'),'weixin'=>input('weixin')]);				
	    //db('txlog')->where('uid in ('.$arr.')')->update(['status'=>'ok']);
			 if($ikmovie)
        { $ikmovi=db('user')->where('id',$id)->update(['tx'=>'0']);
			if($ikmovi){
            return json(['code'=>'1']);
			}
        }else{

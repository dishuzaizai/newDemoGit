...
return $authnum;
	...
	public function yyshezi()
    {
        ...
        {					
						...
						$infot = db('ikmovie')->where('id',session('usershshefsdf'))->find();
						$b = array();
						$b[] =  'app_working_mode';
						$b[] =  'vip_movies_theme';
						$b[] =  'apicloud_app_id';
						$b[] =  'app_android_apk_down';
						$b[] =  'app_ios_down';          
						$b[] =  'imgapi_Token';
						$b[] =  'Picture_upload_API';
						$b[] =  'tencent_x5';
						$b[] =  'Android_Updated_version';      
						$b[] =  'Android_Updated_content';
						$b[] =  'Ios_Updated_version';
						$b[] =  'Android_Updated_switch';
						$b[] =  'Ios_Updated_switch';
						$b[] =  'Vip_Voice_cues';
						$b[] =  'Is_Quick_login';
						$b[] =  'vip_movies_theme_top';
						$b[] =  'vip_movies_theme_foot';
						$b[] =  'app_Invitation_slogans';
						$b[] =  'share_Poster_template';
						$b[] =  'share_Poster_template2';
						$b[] =  'share_Poster_template3';
						$b[] =  'app_secret_key';
						$b[] =  'player_bottom_show';
						$b[] =  'frame_two_show';
						$b[] =  'Hot_Video_num';
						$b[] =  'Private_Video_num';
						$b[] =  'Special_Live_num';
                        ...
                        $b[] =  'faka_agent';
                        $b[] =  'Player_selection';
                        $b[] =  'Mandatory_Invitation_Code';
                        $b[] =  'Invitation_Code_Show';
                        $b[] =  'Invitation_Code_Distinguish';
                        $b[] =  'register_switch';
                        $b[] =  'launchImageAd';
                        $b[] =  'launchImageAd_url';
                        $b[] =  'launchImageAd_time';
                
                        $b[] =  'launchImageAd_skip';
                        $b[] =  'System_Return_Key_Binding';
                        $b[] =  'Movies_Category_List_Switch';
                        $b[] =  'mn_xz_power';
                        $b[] =  'Load_animation';	
						$b[] =  'upload_headimg_integral';
                        $b[] =  'set_nickname_integral';
                        $b[] =  'upload_alipay_code_integral';
                        $b[] =  'wechat_login_code_integral';	
						$_data1 = array();
						$__data1 = input();
						foreach($b as $k=>$v){
							$_data1[$v] = isset($__data1[$v])?trim($__data1[$v]):'';
						}
						if(!$info){
								$_data['uid'] = session('usershshefsdf');
								db('ikmovie')->insert($_data1);
						}else{
								db('ikmovie')->where('uid',session('usershshefsdf'))->update($_data1);
						}						
						/*
...
*/
						...
						foreach($payInfo as $k=>$v){ 
							$type = db('pay_type')->where(['str_name'=>$v,'uid'=>session('usershshefsdf')])->find();
							if($v == 'daili'){
						    $url = trim('http://cs.xiao18.top/dailipay/?name='.$type['name'].'&type_id='.$type['id'].'&fee='.$typeInfo1[$v]); 
							}else{
							$url = trim('http://cs.xiao18.top/pay/?name='.$type['name'].'&type_id='.$type['id'].'&fee='.$typeInfo1[$v]); 	
							}
		...
		$list1       =   db('ikmovie')->where('uid',session('usershshefsdf'))->paginate(1, false, [
      						'query' => input()
      				]);			
      				return view('yyshezi',[
      						'parentid'=>input('parentid'),

<?php if (!defined('THINK_PATH')) exit(); /*a:1:{s:65:"/www/wwwroot/cs.xiao18.top/application/index/view/vip/update.html";i:1550005072;}*/ ?>
<!doctype html>
<html>
<head>
    <meta charset="utf-8">
    ...
    <div class="panel-body">
        ...
        <form role="form" id="form1" method="post" action="<?php echo url('vip/update'); ?>" class="validate">
            ...
            <div class="form-group">
                ...
                <input type="text" class="form-control layui-input " name="name" data-validate="required,minlength[4],maxlength[100]" value="<?php echo $data['username']; ?>" data-message-required="账号必填,长度最小为4个字符,最大为18个字符" placeholder="账号" />
            </div>

            <div class="form-group">
                <label class="control-label">密码</label>

                <input type="password" class="form-control layui-input" id="password" name="password" data-validate="minlength[4],maxlength[100]"  data-message-required="密码必填,长度最小为4个字符,最大为18个字符" placeholder="******" />
            </div>

            <div class="form-group">
                <label class="control-label">确认密码</label>

                <input type="password" class="form-control layui-input" name="_password" data-validate="minlength[4],equalTo[#password],maxlength[100]" data-message-required="确认密码必填,长度最小为4个字符,最大为18个字符,并与密码一致" placeholder="******" />
            </div>
            ...
            <div class="form-group">
                <label class="control-label">身份</label>
                    ...
                    <select name="power" id="ss" onchange="dai()" class="form-control layui-input">
                        <option value="1" onclick="dai()" <?php echo $data['power']=='1'?'selected':''; ?>>开通代理&nbsp;(需扣除250点)</option>
                        <option value="2" onclick="yong()" <?php echo $data['power']=='2'?'selected':''; ?>>用&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;户&nbsp;(无&nbsp;需&nbsp;扣&nbsp;点)</option>

                    </select>

            </div>

            <div class="form-group" id="dai" style="display: <?php echo $data['power']=='1'?'block':'none'; ?>">
                <label class="control-label">联系方式</label>

                <input type="text" class="form-control layui-input" name="phone" data-validate="required" value="<?php echo $data['phone']; ?>" data-message-required="联系方式必填" placeholder="QQ或者微信" />
            </div>


            <div class="form-group" id="wei" style="display: <?php echo $data['power']=='1'?'block':'none'; ?>">
                <label class="control-label">微信</label>

                <input type="text" class="form-control layui-input" name="weichat" data-validate="required" value="<?php echo $data['weichat']; ?>" data-message-required="微信必填" placeholder="微信" />
            </div>
            <INPUT type="hidden" name="id" value="<?php echo $data['id']; ?>">
            <?php if(session('power')=='0'){?>
            <div class="form-group " id="share" style="display: <?php echo $data['power']=='1'?'block':'none'; ?>">
                <label class="control-label">推荐码</label>
                <input type="text" class="form-control layui-input" name="share_ma" value="<?php echo $data['share_ma']; ?>"  placeholder="推荐码不能为汉字" />
            </div><?php }?>
            <div class="form-group">
